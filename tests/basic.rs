...
#[allow(unused)]
const HYPERVISOR_NOJAILER_CONFIG: LazyLock<HypervisorConfig> = LazyLock::new(|| {
    HypervisorConfig {
        id: env::var("ID")
            .ok()
...
#[allow(unused)]
const HYPERVISOR_WITHJAILER_CONFIG: LazyLock<HypervisorConfig> =
    LazyLock::new(|| HypervisorConfig {
        id: env::var("ID")
            .ok()
            .and_then(|s| Some(s + &uuid::Uuid::new_v4().to_string())),
        poll_status_secs: 20,
...
#[allow(unused)]
const MICROVM_CONFIG: LazyLock<MicroVMConfig> = LazyLock::new(|| MicroVMConfig {
    logger: Some(Logger {
        level: None,
