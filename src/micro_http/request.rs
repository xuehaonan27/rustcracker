use std::{fs::File, io::{Write, Error as WriteError}, str::from_utf8};

use hyper::body;

use super::{ascii::{CR, CRLF_LEN, LF, SP}, find, Body, RequestHeaders, Method, RequestError, Version};
...
impl RequestLine {
    ...
    fn write_all<T: Write>(&self, mut buf: T) -> Result<(), WriteError> {
        ...
        buf.write_all(&[SP])?;
        buf.write_all(self.uri.string.as_bytes())?;
        buf.write_all(&[SP])?;
...
impl Request {
    /// Creates a new HTTP `Request` with an empty body.
    pub fn new(method: Method, uri: Uri, http_version: Version) -> Self {
        let headers = RequestHeaders::default();
        
        Self { request_line: RequestLine::new(method, uri, http_version), headers, body: Default::default()}
    }
    ...
    pub fn set_body(&mut self, body: Body) {
        self.headers.set_content_length(body.len() as u32);
        self.body = Some(body);
    }

    /// Updates the content length of the `Request`.
    ...
    pub fn try_from(byte_stream: &[u8], max_len: Option<usize>) -> Result<Self, RequestError> {
        ...
        match find(&byte_stream[request_line_end..], &[CR, LF, CR, LF]) {
            ...
            Some(headers_end) => {
                ...
                let headers = RequestHeaders::try_from(&headers_and_body[..headers_end])?;
