...
const JAILER_CONFIG: LazyLock<JailerConfig> = LazyLock::new(|| JailerConfig {
...
pub(crate) const HYPERVISOR_NOJAILER_CONFIG: LazyLock<HypervisorConfig> = LazyLock::new(|| {
    HypervisorConfig {
        id: env::var("ID")
            .ok()
...
pub(crate) const HYPERVISOR_WITHJAILER_CONFIG: LazyLock<HypervisorConfig> = LazyLock::new(|| {
    HypervisorConfig {
        id: env::var("ID")
            .ok()
            .and_then(|s| Some(s + &uuid::Uuid::new_v4().to_string())),
...
const BOOT_SOURCE: LazyLock<BootSource> = LazyLock::new(|| BootSource {
    boot_args: Some("console=ttyS0 reboot=k panic=1 pci=off".to_string()),
    initrd_path: None,
    kernel_image_path: env::var("KERNEL_IMAGE_PATH").expect("kernel image path not found"),
});

const ROOTFS: LazyLock<Drive> = LazyLock::new(|| Drive {
...
pub(crate) const MICROVM_CONFIG: LazyLock<MicroVMConfig> = LazyLock::new(|| MicroVMConfig {
    logger: Some(Logger {
        level: None,
